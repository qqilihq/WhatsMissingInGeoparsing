...
# -------------------------------------------CORPUS STATISTICS----------------------------------------------------------
# which countries most popular, unique pages, which features most popular, plus the other ones

o = codecs.open("./wiktor.txt", mode="w", encoding="utf-8")
tree = ElementTree.parse("./wiktor.xml")
for node in tree.getroot():
    text = node.find("./text").text
    o.write(text)
o.close()
